...
public class TramaUtil {
	private static LoggerSati log3 = LoggerSati.getInstance(TramaUtil.class.getName());
	...
	public static String getMapValue(String decimal){
		...
		if (value==null){
			//if(Constante.VER_LOG) log.info("decimal no encontrado="+decimal);
			return " ";
		}
		//if(Constante.VER_LOG) log.info("decimal no encontrado="+decimal);		
		return value;
	...
	//	String cadenucha = "";
		...
		for (int i = 0,j=0; i < tramacodificada.length(); j++) {
		    int n = i+3; 
			bytes[j] = ObjectUtil.positiveIntToByte(Integer.parseInt(tramacodificada.substring(i,n)));
			i=i+3;
		//	cadenucha += String.valueOf(bytes[j])+ "\r\n";
		}
		
		//printWithName(cadenucha.getBytes(),"305");
		return bytes;
	}
...
*/
	...
	/***
	 * metodo encargado de entregar los bytes transmitidos en una cadena.
	 */
	public static String getProcessBytes(byte[] datos) { 
        String resultado=""; 
        ByteArrayInputStream input = new ByteArrayInputStream(datos); 
        String cadAux; 
        int leido = input.read(); 
        while(leido != -1) {
                resultado += (char)leido; 
                leido = input.read(); 
        }
    return resultado; 
	}
	...
	private static void printWithName(byte[] trama,String nombre){
	    File f = null;
