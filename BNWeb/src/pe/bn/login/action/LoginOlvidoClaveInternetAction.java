...
public class LoginOlvidoClaveInternetAction extends GrandActionAbstract{
	private static LoggerSati log3 = LoggerSati.getInstance(LoginOlvidoClaveInternetAction.class.getName());
	...
	public ActionForward iniciar(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String valtit = request.getSession().getAttribute("titulo").toString();
		
		System.out.println("valtit:::"+valtit);
		...
		return mapping.findForward("iniciaOlvido");
	}
	...
	public ActionForward generarClaveInternet(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			request.getSession().setAttribute(ConstanteSesion.TARJETA,FacadeFactory.getTarjetaFacade().generarNuevaClaveOlvidoInternet(usuario,transaccion,passwordEncripClave6,pinblock1,pinblock2, request.getRemoteAddr(),request));
	...
	String idObjeto = "mailAutoOlvidoClaveInternet";
