...
public class LoginExpiraClaveInternetAction extends GrandActionAbstract{
	private static LoggerSati log3 = LoggerSati.getInstance(LoginExpiraClaveInternetAction.class.getName());
	...
	public ActionForward iniciar(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		return mapping.findForward("iniciaExpira");
	}
	...
	public ActionForward generarClaveInternet(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String claveInternetActual	= request.getParameter("txtClaveInternetActual");
			String claveInternet	= request.getParameter("txtClaveInternet");
			String claveInternet_	= request.getParameter("txtClaveInternet_");
			String transaccion		= Constante.AFILIACION_INTERNET_OLVIDO;
			
			String dencriptClaveTarjeta 	= getClaveDesencriptada(claveInternetActual,request);
			...
			//MGL-ACA
			//request.getSession().setAttribute(ConstanteSesion.TARJETA,FacadeFactory.getTarjetaFacade().generarExpiraClaveInternet(usuario,transaccion,passwordEncripClave6,pinblock1,pinblock2, request.getRemoteAddr(),request));
			
			Tarjeta tarjeta= FacadeFactory.getTarjetaFacade().getCambiarClaveExterna(ConstanteSesion.CODIGO_TRANSACCION_CAMBIO_CLAVE_EXTERNO,usuario, dencriptClaveTarjeta, dencriptclaveInternet,  request);
			...
			//request.getSession().setAttribute(ConstanteSesion.TARJETA,FacadeFactory.getTarjetaFacade().generarOlvidoClaveInternet(usuario,transaccion,passwordDesencrip,dencriptclaveInternet,dencriptclaveInternet_, request.getRemoteAddr(),request));
