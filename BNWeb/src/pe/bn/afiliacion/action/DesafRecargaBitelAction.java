...
public class DesafRecargaBitelAction extends GrandActionAbstract {
	private static LoggerSati log3 = LoggerSati.getInstance(DesafRecargaBitelAction.class.getName());
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
			...
			request.getSession().setAttribute("TITULO",Constante.DESAF_BITEL_TITULO);
			listaDesafiliacion =  FacadeFactory.getAfiliacionFacade().getAfiliaciones(Constante.RECARGA_BIT,usuario.getTarjeta().getNumero(), "00438");
		
		request.getSession().setAttribute("listaDesafiliacion",listaDesafiliacion);
		return mapping.findForward("iniciarDesafBitel");
	}
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
		    ...
		    request.setAttribute("hidServicio", "14");
		    path = request.getContextPath()+"/recargaTelefono.do?hidServicio=14";
