 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class DesafCuponesAction extends GrandActionAbstract {
	private static LoggerSati log3 = LoggerSati.getInstance(DesafCuponesAction.class.getName());
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Referencia:");
		    ...
		    ConstanteSesion.TITULO = Constante.DESAF_CUPONES_TITULO;
			request.getSession().setAttribute("TITULO",Constante.DESAF_CUPONES_TITULO);
			//listaDesafiliacion =  FacadeFactory.getAfiliacionFacade().getAfiliaciones(Constante.PAGO_CUP,usuario.getTarjeta().getNumero(), "09043");
			listaDesafiliacion =  FacadeFactory.getAfiliacionFacade().getAfiliaciones(hidCodEntidad,usuario.getTarjeta().getNumero(), "09043");
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Referencia");
		    ...
		    path = request.getContextPath()+"/pagoCupones.do?hidServicio=9";
