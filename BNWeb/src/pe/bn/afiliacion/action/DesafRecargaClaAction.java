...
public class DesafRecargaClaAction extends GrandActionAbstract {
	private static LoggerSati log3 = LoggerSati.getInstance(DesafRecargaClaAction.class.getName());
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
			...
			request.getSession().setAttribute("TITULO",Constante.DESAF_CLARO_TITULO);
			listaDesafiliacion =  FacadeFactory.getAfiliacionFacade().getAfiliaciones(Constante.RECARGA_CLA,usuario.getTarjeta().getNumero(), "00438");
		
		request.getSession().setAttribute("listaDesafiliacion",listaDesafiliacion);
		return mapping.findForward("iniciarDesafClaro");
	}
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
		    ...
		    request.setAttribute("hidServicio", "9");
		    path = request.getContextPath()+"/recargaTelefono.do?hidServicio=13";
