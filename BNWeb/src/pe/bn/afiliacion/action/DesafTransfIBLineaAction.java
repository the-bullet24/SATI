...
public class DesafTransfIBLineaAction extends GrandActionAbstract {
	private static LoggerSati log3 = LoggerSati.getInstance(DesafTransfIBLineaAction.class.getName());
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
			...
			ConstanteSesion.TITULO = "DESAFILIACION TRANSFERENCIAS INTERBANCARIAS EN LINEA";
			request.getSession().setAttribute("TITULO","DESAFILIACION TRANSFERENCIAS INTERBANCARIAS EN LINEA");
			...
			listaDesafiliacion =  FacadeFactory.getAfiliacionFacade().getAfiliaciones(Constante.TB_INTERBANCO_LINEA,usuario.getTarjeta().getNumero(), "00004");
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
		    ...
		    path = request.getContextPath()+"/transferenciaInterbancariaLinea.do";
