...
public class DesafRecargaMovAction extends GrandActionAbstract {
	
	private static LoggerSati log3 = LoggerSati.getInstance(DesafRecargaMovAction.class.getName());
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
			...
			listaDesafiliacion =  FacadeFactory.getAfiliacionFacade().getAfiliaciones(Constante.RECARGA_MOV,usuario.getTarjeta().getNumero(), "00437");
		
		request.getSession().setAttribute("listaDesafiliacion",listaDesafiliacion);
		return mapping.findForward("iniciarDesafMovistar");
	}
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
		    ...
		    request.setAttribute("hidServicio", "8");
		    path = request.getContextPath()+"/recargaTelefono.do?hidServicio=8";
