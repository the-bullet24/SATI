...
public class DesafTarjetaBNAction extends GrandActionAbstract {
	private static LoggerSati log3 = LoggerSati.getInstance(DesafTarjetaBNAction.class.getName());
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
		    ...
		    ConstanteSesion.TITULO = "DESAFILIACION DE TARJETAS DE CREDITO BN";
			request.getSession().setAttribute("TITULO","DESAFILIACION TARJETAS DE CREDITO BN");
			...
			listaDesafiliacion =  FacadeFactory.getAfiliacionFacade().getAfiliaciones(Constante.PAGO_TARJETA_BN,usuario.getTarjeta().getNumero(), "00004");
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
		    ...
		    path = request.getContextPath()+"/pagoTarjetaOtrosBN.do";
	...
	public ActionForward finDesafiliar(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
   		for(int i =0; i<nNum; i++)
   		{
   			...
   			Afiliacion af = FacadeFactory.getAfiliacionFacade().getAfiliacion(id[0],id[1],new Long(id[2]), "00004");
   			...
   			af.setTipoAfiliacion(id[0]);
				...
				if(element.getSecuencia().toString().trim().equals(af.getSecuencia().toString().trim())){
					...
					af.setBeneficiario(element.getBeneficiario());
