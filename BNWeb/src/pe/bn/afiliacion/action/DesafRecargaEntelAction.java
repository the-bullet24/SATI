...
public class DesafRecargaEntelAction extends GrandActionAbstract {
	private static LoggerSati log3 = LoggerSati.getInstance(DesafRecargaEntelAction.class.getName());
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
		    ...
		    //ConstanteSesion.TITULO = Constante.DESAF_CLARO_TITULO;
			request.getSession().setAttribute("TITULO",Constante.DESAF_ENTEL_TITULO);
			listaDesafiliacion =  FacadeFactory.getAfiliacionFacade().getAfiliaciones(Constante.RECARGA_ENT,usuario.getTarjeta().getNumero(), "00438");
		
		request.getSession().setAttribute("listaDesafiliacion",listaDesafiliacion);
		return mapping.findForward("iniciarDesafEntel");
	}
	...
	throws Exception {
		...
		request.getSession().setAttribute(ConstanteSesion.DES_SUBTITULO2,"Nro. de Servicio");
		    ...
		    request.setAttribute("hidServicio", "16");
		    path = request.getContextPath()+"/recargaTelefono.do?hidServicio=16";
