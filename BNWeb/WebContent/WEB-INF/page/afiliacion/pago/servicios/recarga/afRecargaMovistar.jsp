	  ...
	  // Validando la clave de coordenada
		...
		if(tipoElemento == '5')
		{
			...
			if (validalongitud("txtCoordenada","6")){
				alert('La clave dinámica del Token es de 6 dígitos, no menos');
				return;
			}
		}else{
			if (validacampo("txtCoordenada")){
		...
		frmAfilPagoServicio.action="<%=request.getContextPath()%>/afilRegarcaMov.do?metodo=afiliar";
					...
					<c:if test="${tipoElemento.tipoElementoSeguridad=='2'}">
						<TR bgcolor="#e5e5de">
							...
							<TD height="20" colspan="2" bgcolor="#e5e5de" width="355"><span class="textizqn"><B>
								Ingresar la Siguiente Coordenada </B></span><br><LABEL><B>&nbsp; </B><FONT size="+1">
								<SPAN style="color: #a80000"><B><c:if test="${resultCoord.coordConcat eq null}">No disponible</c:if>
								 ...
								 <INPUT type="password" name="txtCoordenada" size="6" maxlength="2" <c:if test="${resultCoord.coordConcat eq null}"> readonly="readonly"</c:if> onkeypress="return soloNumerosAll(event)"><BR>
										<FONT size="-1" face="Arial"><FONT size="-1" face="Arial"><B></B></font></font>
							</TD>
						</TR>
					</c:if>
					<c:if test="${tipoElemento.tipoElementoSeguridad=='5'}">
						<TR bgcolor="#E5E5DE">
							<TD  height="20" colspan="2" bgcolor="#e5e5de" width="140"><span class="textizqn"><B>
							...
							<TD colspan="2" height="20"  bgcolor="#e5e5de" width="355"><img
					...
					<c:if test="${tipoElemento.tipoElementoSeguridad=='2'}">
						<TR>
							<TD COLSPAN="4"  height="20" width="40%" class="textizqn"  bgcolor="#e5e5de"><b><u>Ejemplo:</u>
								...
								6</b> y la columna de la <B>letra  F</B>, en la unión de ambos, obtendrás un número, éste número deberás ingresarlo para aprobar la operación.
							</TD>
						</TR>
					</c:if>
					<c:if test="${tipoElemento.tipoElementoSeguridad=='5'}">
