...
<SCRIPT language="javascript">
	...
	function imprimir(){
		window.open('<%=request.getContextPath()%>/util.do?metodo=solicitarRefrendo&variableSesion=<%=pe.cosapi.common.ConstanteSesion.TARJETA%>&idObjeto=refrendoExpiraClaveInternet',"BN","toolbar=0,location=0,width=480,height=500, scrollbars=yes, resizable=yes,  top=" + ((screen.height/2)-(500/2))+", left="+((screen.width/2)-(470/2)));
	}
	
	function enviar(){
		window.open('<%=request.getContextPath()%>/util.do?metodo=verMail&variableSesion=<%=pe.cosapi.common.ConstanteSesion.TARJETA%>&idObjeto=mailExpiraClaveInternet',"mail","toolbar=0,location=0,width=480,height=580, scrollbars=yes, resizable=yes,  top=" + ((screen.height/2)-(500/2)-20)+", left="+((screen.width/2)-(470/2)));
	}
 	...
 	function verPdf(){
		...
		form.idObjeto.value = 'refrendoExpiraClaveInternet';
					 ...
					 <h4 style="font-weight:bold; size:16px;text-align: center;"><bean:write name="<%=pe.cosapi.common.ConstanteSesion.USUARIO_EN_SESION%>" property="nombre"/></h4>
						...
						<div id="contenidos-informativos">
							...
							<caption class="titulo-constancia1">
								CONSTANCIA DE GENERACION POR EXIRACIÓN DE CLAVE INTERNET
						    </caption>
						    <tbody>
						    <tr>
								<td>Nro. Tarjeta:</td>
