...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/navegadores.js"></script>
	...
	<title>Generacion por Olvido de Clave Internet. Banco de la Nación</title>
...
<SCRIPT language="javascript">
	function salir(){
	location.href='<%=request.getContextPath()%>/Inicio';		

	}
	
	function imprimir(){
		window.open('<%=request.getContextPath()%>/util.do?metodo=solicitarRefrendo&variableSesion=<%=pe.cosapi.common.ConstanteSesion.TARJETA%>&idObjeto=refrendoOlvidoClaveInternet',"BN","toolbar=0,location=0,width=480,height=500, scrollbars=yes, resizable=yes,  top=" + ((screen.height/2)-(500/2))+", left="+((screen.width/2)-(470/2)));
	}
	
	function enviar(){
		window.open('<%=request.getContextPath()%>/util.do?metodo=verMail&variableSesion=<%=pe.cosapi.common.ConstanteSesion.TARJETA%>&idObjeto=mailOlvidoClaveInternet',"mail","toolbar=0,location=0,width=480,height=580, scrollbars=yes, resizable=yes,  top=" + ((screen.height/2)-(500/2)-20)+", left="+((screen.width/2)-(470/2)));
	}
 	...
 	function verPdf(){
		...
		form.idObjeto.value = 'refrendoOlvidoClaveInternet';
        ...
        <div id="cabecera">
            ...
            <div id="login">
                ...
                <div id="login-contenido">
					...
					<h1 class="dax"><span>Generación por Olvido de Clave Internet</span></h1>
					 ...
					 <h4 style="font-weight:bold; size:16px;text-align: center;"><bean:write name="<%=pe.cosapi.common.ConstanteSesion.USUARIO_EN_SESION%>" property="nombre"/></h4>
						...
						<div id="contenidos-informativos">
							...
							<caption class="titulo-constancia1">
								CONSTANCIA DE GENERACION POR OLVIDO DE CLAVE INTERNET
						    </caption>
						    <tbody>
						    <tr>
								<td>Nro. Tarjeta:</td>
