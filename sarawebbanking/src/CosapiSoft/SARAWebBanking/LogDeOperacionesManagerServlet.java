 ...
 */
public class LogDeOperacionesManagerServlet extends JavaServlet {
/**
 * AyudaDeCampoServlet constructor comment.
 */
public LogDeOperacionesManagerServlet() {
	super();
}
...
public void doPost(javax.servlet.http.HttpServletRequest req, javax.servlet.http.HttpServletResponse res) throws java.io.IOException {
	try {
		...
		if (req.getSession(false).getAttribute("logManager") != null) {
			bean = ((LogDeOperaciones) req.getSession(false).getAttribute("logManager"));
		}
		else {
			bean = new LogDeOperaciones();
			req.getSession(false).setAttribute("logManager", bean);
		}
		bean.setLogin((CosapiSoft.SARAWebBanking.InicioSesion) req.getSession(false).getAttribute(modulo));
		bean.setModulo(modulo);
		bean.setError("");
		
		bean.getLogin().setNamePantalla("SEGURIDAD DE LOG DE OPERACIONES");
		...
		if (value.equals("Cargar Tabla")) {
			...
			bean.getLogin().setCodact("318");
			bean.getLogin().setAccion("Cargar");
			if (!bean.getLogin().hasAccesAccion()) {
				//				res.sendRedirect(JspServlet.ERROR_MANAGER_JSP);
				callPage(req, res, JspServlet.ERROR_MANAGER_JSP);
				return;
