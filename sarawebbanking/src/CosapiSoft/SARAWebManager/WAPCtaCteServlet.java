 ...
 */
public class WAPCtaCteServlet extends JavaServlet {
	
/**
 * AyudaDeCampoServlet constructor comment.
 */
public WAPCtaCteServlet() {
	super();
}
...
public void doPost(javax.servlet.http.HttpServletRequest req, javax.servlet.http.HttpServletResponse res) throws java.io.IOException {
	...
	try {
		...
		WAPCtaCte bean;
		if (req.getSession(false).getAttribute("ctacte") != null) {
			bean = ((WAPCtaCte) req.getSession(false).getAttribute("ctacte"));
		}
		else {
			bean = new WAPCtaCte();
			bean.setLogin((CosapiSoft.SARAWebBanking.InicioSesion) req.getSession(false).getAttribute("manager"));
			req.getSession(false).setAttribute("ctacte", bean);
		}
		
		bean.getLogin().setError("");
		bean.getLogin().setNamePantalla("SALDO CUENTA CORRIENTE");
		System.out.println("Mensaje Opción:"+bean.getLogin().getNamePantalla());
		
		if (value.equals("Inicio")) {
			bean.getLogin().setCodact("335");
		bean.getLogin().setAccion("Consulta");
			...
			bean.setGrid(null);
			bean.getLogin().setError("");
			callPage(req, res, JspServlet.WAP_CTA_CORRIENTE_JSP_JSP);
			return;
		}
