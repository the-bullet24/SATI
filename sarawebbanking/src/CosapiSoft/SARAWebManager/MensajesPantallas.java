...
public class MensajesPantallas {
	//
...
public void agregar() throws Exception {
	...
	try {
		error = "";
		if (!buscar(db)) {
			PreparedStatement preparedStatement= jd.getConnection().prepareStatement("INSERT INTO "+Constante.ESQUEMA1+".TMSGPAN (codgrp, codmsg, desmsg, " + 
                  " idemsg) VALUES (?, ?, ?, ?)");
...
/*
			String query = //
			"Insert into "+Constante.ESQUEMA1+".TMSGPAN values ('" + getCodgrp() + "', '" + getCodmsg() + "', '" + getDesmsg() + "', '" + getIdemsg() + "')";
			...
			LogOpe.saveLogManager(db, login.getNameProducto(), login.getUsuario(), "Agregar", Constante.ESQUEMA1+".TMSGPAN", Table.getColumnsTmsgpan(), bef, aft);
			loadGrid(db);
		} else {
...
private boolean buscar(JQuery db) throws Exception {
	try {
		...
		"From "+Constante.ESQUEMA1+".TMSGPAN " + //
...
public void consultar() throws Exception {
	try {
		String bef[] = {"", "", "", ""};
		String aft[] = {"", "", "", ""};
		LogOpe.saveLogManager(login.getNameProducto(), login.getUsuario(), "Consultar", Constante.ESQUEMA1+".TMSGPAN", Table.getColumnsTmsgpan(), bef, aft);
	} catch (Exception e) {
...
public void eliminar() throws Exception {
	...
	try {
		...
		if (buscar(db)) {
			String query = //
			"Delete from "+Constante.ESQUEMA1+".TMSGPAN " + //
			"Where codgrp = '" + getCodgrp() + "' " + //
			"And codmsg = '" + getCodmsg() + "' ";
			db.setQuery(query);
			db.executeUpdate();
			...
			String aft[] = {"", "", "", ""};
			LogOpe.saveLogManager(db, login.getNameProducto(), login.getUsuario(), "Eliminar", Constante.ESQUEMA1+".TMSGPAN", Table.getColumnsTmsgpan(), bef, aft);
			loadGrid(db);
		} else {
...
/**
 * This method was created in VisualAge.
 * @return java.lang.Stringzjkxc
 */
public String getCodmsg() {
	if (codmsg == null || codmsg.equals(""))
		codmsg = "--- ";
	CString str = new CString(codmsg);
	str.arrange();
	return str.toString();
}
/**
...
public String getUrlMensajesPantallas() {
	return JspServlet.MENSAJES_PANTALLAS_SERVLET + "?BtnDatos=Buscar&TxtCodgrp=" + getCodgrp() + "&TxtCodmsg=";
}
...
private void loadGrid(JQuery db) throws Exception {
	// System.out.println("llegó aki al loadgrid");
...
public void modificar() throws Exception {
	...
	try {
		...
		if (buscar(db)) {
			String query = //
			"Update "+Constante.ESQUEMA1+".TMSGPAN set " + //
			"idemsg = '" + antIdemsg + "', " + //
			"desmsg = '" + antDesmsg + "' " + //
			"Where codgrp = '" + getCodgrp() + "' " + //
			"And codmsg = '" + getCodmsg() + "' ";
			...
			LogOpe.saveLogManager(db, login.getNameProducto(), login.getUsuario(), "Modificar", Constante.ESQUEMA1+".TMSGPAN", Table.getColumnsTmsgpan(), bef, aft);
			loadGrid(db);
		} else {
			String str = Mensajes.getMessage(Mensajes.CODIGO_DE_MENSAJE_NO_EXISTE);
			int pos = str.indexOf('&');
