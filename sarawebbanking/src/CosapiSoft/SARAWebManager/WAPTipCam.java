...
private void loadGrid(JQuery db) throws Exception {
	try {
		getGrid().clear();
		String query = //
		"Select to_char(datpro,'dd/mm/rrrr'),flgcom,msghst " + //
		"From "+Constante.ESQUEMA2+".HJOUTRA " + //
		"Where codtrahst in ('3800') and flgcom in ('2','3') "+ //
		getQueryFecha() + //
		getQueryEstado() + //
...
public void next(int i) {
	...
	datpro = ((java.util.Vector) getGrid().elementAt(i)).elementAt(0).toString();
	empresa = ((java.util.Vector) getGrid().elementAt(i)).elementAt(1).toString();
	if(empresa.equals("2")) empresa="Telefónica";
	if(empresa.equals("3")) empresa="Claro";
	msgerror = ((java.util.Vector) getGrid().elementAt(i)).elementAt(2).toString();
