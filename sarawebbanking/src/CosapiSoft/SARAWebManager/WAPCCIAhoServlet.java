 ...
 */
public class WAPCCIAhoServlet extends JavaServlet {
	
/**
 * AyudaDeCampoServlet constructor comment.
 */
public WAPCCIAhoServlet() {
	super();
}
...
public void doPost(javax.servlet.http.HttpServletRequest req, javax.servlet.http.HttpServletResponse res) throws java.io.IOException {
	...
	try {
		...
		WAPCCIAho bean;
		if (req.getSession(false).getAttribute("cciaho") != null) {
			bean = ((WAPCCIAho) req.getSession(false).getAttribute("cciaho"));
		}
		else {
			bean = new WAPCCIAho();
			bean.setLogin((CosapiSoft.SARAWebBanking.InicioSesion) req.getSession(false).getAttribute("manager"));
			req.getSession(false).setAttribute("cciaho", bean);
		}
		
		bean.getLogin().setError("");
		bean.getLogin().setNamePantalla("CCI CUENTA DE AHORRO");
		System.out.println("Mensaje Opción:"+bean.getLogin().getNamePantalla());
		
		if (value.equals("Inicio")) {
			bean.getLogin().setCodact("337");
			...
			bean.setGrid(null);
			bean.getLogin().setError("");
			callPage(req, res, JspServlet.WAP_CCI_AHORROS_JSP);
			return;
		}
