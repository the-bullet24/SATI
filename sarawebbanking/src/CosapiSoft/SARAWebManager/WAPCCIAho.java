...
public class WAPCCIAho extends GenericTable{
	//
...
private String getQueryNroCta() {
	if (getNrotar().equals(""))
		return "";
	String query = " And numprdsrc = '" + getNrotar() + "' ";
	return query;
}
...
public void loadGrid(JQuery db) throws Exception {
	try {
		
		String query = //
		"Select numprdsrc,to_char(datpro,'DD/MM/YYYY HH24:MI:SS'),flgcom,nrocta,msghst,numprdtar " + //
		"From "+Constante.ESQUEMA2+".HJOUTRA " + //
		"Where codtrahst in ('1125') and flgcom in ('2','3') "+ //
		getQueryFecha() + //
		getQueryNroCta() + //
		getQueryEstado() + //
...
public void next(int i) {
	nrotar = ((java.util.Vector) getGrid().elementAt(i)).elementAt(0).toString();
	datpro = ((java.util.Vector) getGrid().elementAt(i)).elementAt(1).toString();
	empresa = ((java.util.Vector) getGrid().elementAt(i)).elementAt(2).toString();
	if(empresa.equals("1")) empresa="ATI";
	else if(empresa.equals("2")) empresa="Telefónica";
	else if(empresa.equals("3")) empresa="Claro";
	numcta = ((java.util.Vector) getGrid().elementAt(i)).elementAt(3).toString();
	msgerror = ((java.util.Vector) getGrid().elementAt(i)).elementAt(4).toString();
	if(msgerror.equals("") || msgerror==null) msgerror="Satisfactorio";
	else msgerror="Fallido";
	numref = ((java.util.Vector) getGrid().elementAt(i)).elementAt(5).toString();
}
...
public void setModulo(String newValue) {
	...
	/**
	 * @param nrotar El nrotar a establecer.
	 */
