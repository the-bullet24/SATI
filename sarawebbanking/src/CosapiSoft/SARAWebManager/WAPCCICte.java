...
private void loadGrid(JQuery db) throws Exception {
	try {
		getGrid().clear();
		String query = //
		"Select numdoc,coddoc,to_char(datpro,'DD/MM/YYYY HH24:MI:SS'),flgcom,nrocta,codtrahst,msghst,numprdtar " + //
		"From "+Constante.ESQUEMA2+".HJOUTRA " + //
		"Where codtrahst in ('0125') and flgcom in ('2','3') "+ //
		getQueryFecha() + //
		getQueryNroCta() + //
		getQueryDoc() + //
		getQueryEstado() + //
...
public void next(int i) {
	...
	else msgerror="Fallida";
	numref = ((java.util.Vector) getGrid().elementAt(i)).elementAt(7).toString();
