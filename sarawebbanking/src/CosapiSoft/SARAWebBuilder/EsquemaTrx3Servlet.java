...
public class EsquemaTrx3Servlet extends JavaServlet {
/**
 * AyudaDeCampoServlet constructor comment.
 */
public EsquemaTrx3Servlet() {
	super();
}
public void doPost(javax.servlet.http.HttpServletRequest req, javax.servlet.http.HttpServletResponse res) throws java.io.IOException, javax.servlet.ServletException {
	...
	try {
		...
		bean.getLogin().setNamePantalla(Mensajes.getMessage(Mensajes.ASIG_DE_METODOS_ESQUEMA));
		...
		if (value.equalsIgnoreCase("Regresar")) {
			...
			back = (back.equals("")) ? JspServlet.ESQUEMA_TRX2_JSP : back;
			callPage(req, res, back);
			return;
		}
		Transacciones.push(JspServlet.ESQUEMA_TRX3_JSP);
		callPage(req, res, JspServlet.ESQUEMA_TRX3_JSP);
	}
	catch (Exception e) {
		try {
			e.printStackTrace();
